...
public class CustomSocketFactory extends SSLSocketFactory {
     ...
     * @param compatible
     */
    public CustomSocketFactory(SSLSocketFactory delegate, boolean compatible) {
        this.delegate = delegate;
    ...
    private Socket makeSocketSafe(Socket socket, String host) {
        if (socket instanceof SSLSocket) {
            CustomSSLSocket tempSocket=
                    new CustomSSLSocket((SSLSocket) socket, compatible);
    ...
    private class CustomSSLSocket extends DelegateSSLSocket {

        final boolean compatible;

        @RequiresApi(api = Build.VERSION_CODES.Q)
        private CustomSSLSocket(SSLSocket delegate, boolean compatible) {
            super(delegate);
            this.compatible = compatible;
            Log.i(TAG,"CustomSSLSocket");
            ...
            SSLParameters sp = super.getSSLParameters();
            String a = sp.getEndpointIdentificationAlgorithm();
            Log.i(TAG,">"+a);
            String[] cc =sp.getCipherSuites();
            for(String c:cc)
                Log.i(TAG,"cs:"+c);
            String[] ap =sp.getApplicationProtocols();
            for(String c:ap)
                Log.i(TAG,"ap:"+c);
            SSLSession ss = super.getHandshakeSession();
            try {
                X509Certificate[] certs = ss.getPeerCertificateChain();
