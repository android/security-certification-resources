...
public class SSLPinningTrustManager implements X509TrustManager {
    ...
    public void checkServerTrusted(X509Certificate[] chain, String authType) throws java.security.cert.CertificateException {
        try {
            ...
            try {
                Log.i("Tag","cert exception=>");
                // Ordering issue?
                X509Certificate[] reorderedChain = reorderCertificateChain(chain);
                Log.i("Tag","cert exception=>"+chain);
                Log.i("Tag","cert exception=>"+chain.length);

                if (! Arrays.equals(chain, reorderedChain)) {
                    checkServerTrusted(reorderedChain, authType);
                    Log.i("Tag","checkServer Trusted");
                    return;
                }
                for (int i = 0; i < chain.length; i++) {
                    Log.i("Tag","validate cert=>"+reorderedChain[i].toString());
