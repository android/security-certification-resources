...
public class Util {
     ...
     * is not yet assigned.
     */
    public static final int SDK_INT = VERSION.SDK_INT;//IS_RUNNING_U ? VERSION_CODES.CUR_DEVELOPMENT : VERSION.SDK_INT;
    ...
    public static void requireAndroidS() {
        if (!isAtLeastS()) {
            throw new IllegalStateException("requires API level S, device's on " + VERSION.SDK_INT);
        }
    }
